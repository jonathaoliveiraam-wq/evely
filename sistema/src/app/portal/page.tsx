...
export default function PortalAtivoPage() {
  ...
  async function loadSessions(packageIds: string[]) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: allPkgs } = await supabase
        ...
        .eq("patient_id", pat.id).order("created_at", { ascending: false });

      const latestPkg = (allPkgs ?? [])[0] ?? null;
      if (latestPkg) setPkg(latestPkg);

      if (allPkgs && allPkgs.length > 0) {
        await loadSessions(allPkgs.map(p => p.id));
      }
      setLoading(false);
    })();
  ...
  if (!pkg) {
    const histSessions = sessions.filter(s => s.status === "completed");
    ...
    // Paciente liberado — exibe prontuário completo
    const totalPts = histSessions.reduce((acc, s) =>
      acc + 20 + (s.was_punctual ? 10 : 0) + (s.did_activities ? 10 : 0), 0);
      ...
      <div style={{ maxWidth: 520, margin: "0 auto", padding: "24px 16px 60px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ position: "relative", display: "inline-block", marginBottom: 12 }}>
            <div
              ...
              style={{
                ...
                border: "3px solid #ccfbf1", overflow: "hidden", position: "relative",
                display: "flex", alignItems: "center", justifyContent: "center",
                cursor: "pointer", boxShadow: "0 2px 12px rgba(15,118,110,0.18)",
              }}
            >
              {avatarUrl
                ? <img src={avatarUrl} alt="Foto" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                : <span style={{ fontSize: 34, color: "#fff", fontWeight: 700 }}>{patient?.full_name?.charAt(0)?.toUpperCase() ?? "?"}</span>
              }
              {avatarUploading && (
            ...
            <button onClick={() => !avatarUploading && fileRef.current?.click()} disabled={avatarUploading}
              style={{ position: "absolute", bottom: 2, right: 2, width: 26, height: 26, borderRadius: "50%", background: "#0f766e", border: "2px solid #fff", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 13, padding: 0, color: "#fff" }}>
              📷
            </button>
          ...
          <h2 style={{ fontFamily: "var(--font-display, serif)", color: "#0f766e", fontSize: 21, margin: "0 0 4px" }}>
            {patient?.full_name?.split(" ")[0]}, seu prontuário
          </h2>
          <p style={{ color: "#94a3b8", fontSize: 13 }}>Acesse seu histórico de sessões a qualquer hora.</p>
        </div>

        {/* Resumo */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "14px 16px", boxShadow: "0 1px 6px rgba(0,0,0,0.07)", textAlign: "center" }}>
            <div style={{ fontSize: 28, fontWeight: 900, color: "#0f766e" }}>{histSessions.length}</div>
            <div style={{ fontSize: 12, color: "#94a3b8", marginTop: 2 }}>Sessões concluídas</div>
          </div>
          <div style={{ background: "#fff", borderRadius: 16, padding: "14px 16px", boxShadow: "0 1px 6px rgba(0,0,0,0.07)", textAlign: "center" }}>
            <div style={{ fontSize: 28, fontWeight: 900, color: "#f59e0b" }}>{totalPts}</div>
            <div style={{ fontSize: 12, color: "#94a3b8", marginTop: 2 }}>Pontos acumulados</div>
          </div>
        </div>

        {/* Histórico de sessões */}
        <div style={{ background: "#fff", borderRadius: 18, padding: "18px 20px", boxShadow: "0 1px 6px rgba(0,0,0,0.07)" }}>
          <div style={{ fontSize: 11, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 14 }}>
            Histórico de sessões
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {[...histSessions].reverse().map((s) => (
              <button key={s.id} onClick={() => setHistDetail(s)}
                style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 12, padding: "10px 14px", cursor: "pointer", textAlign: "left", width: "100%" }}>
        ...
        {/* Modal detalhe */}
        {histDetail && (
          <div onClick={(e) => { if (e.target === e.currentTarget) setHistDetail(null); }}
            style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 9999 }}>
            <div style={{ background: "#fff", borderRadius: "20px 20px 0 0", padding: "24px 24px 40px", width: "100%", maxWidth: 520, maxHeight: "85vh", overflowY: "auto" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 800, color: "#0f766e" }}>Sessão {histDetail.session_number}/10</div>
                  <div style={{ fontSize: 12, color: "#94a3b8" }}>
                    {histDetail.completed_at ? new Date(histDetail.completed_at).toLocaleDateString("pt-BR", { day: "2-digit", month: "long", year: "numeric" }) : fmtDate(histDetail.scheduled_date)}
                  </div>
                </div>
                <button onClick={() => setHistDetail(null)} style={{ fontSize: 22, background: "none", border: "none", cursor: "pointer", color: "#94a3b8" }}>×</button>
              </div>
              <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
                <span style={{ padding: "5px 12px", borderRadius: 99, fontSize: 12, fontWeight: 600, background: histDetail.was_punctual ? "#f0fdf4" : "#fff7ed", color: histDetail.was_punctual ? "#16a34a" : "#ea580c", border: `1px solid ${histDetail.was_punctual ? "#86efac" : "#fed7aa"}` }}>
                  ⏰ {histDetail.was_punctual ? "Pontual" : "Com atraso"}
                </span>
                <span style={{ padding: "5px 12px", borderRadius: 99, fontSize: 12, fontWeight: 600, background: histDetail.did_activities ? "#f0fdf4" : "#fff7ed", color: histDetail.did_activities ? "#16a34a" : "#ea580c", border: `1px solid ${histDetail.did_activities ? "#86efac" : "#fed7aa"}` }}>
                  🏠 {histDetail.did_activities ? "Fez atividades" : "Sem atividades"}
                </span>
              </div>
              {histDetail.evolution_notes && <div style={{ marginBottom: 14 }}><div style={{ fontSize: 11, color: "#94a3b8", textTransform: "uppercase", marginBottom: 4 }}>Evolução</div><p style={{ fontSize: 14, color: "#1e293b", lineHeight: 1.6, margin: 0 }}>{histDetail.evolution_notes}</p></div>}
              {histDetail.activities_notes && <div style={{ marginBottom: 14 }}><div style={{ fontSize: 11, color: "#94a3b8", textTransform: "uppercase", marginBottom: 4 }}>Atividades</div><p style={{ fontSize: 14, color: "#1e293b", lineHeight: 1.6, margin: 0 }}>{histDetail.activities_notes}</p></div>}
              {histDetail.recommendation_notes && <div style={{ marginBottom: 14 }}><div style={{ fontSize: 11, color: "#94a3b8", textTransform: "uppercase", marginBottom: 4 }}>Recomendações</div><p style={{ fontSize: 14, color: "#1e293b", lineHeight: 1.6, margin: 0 }}>{histDetail.recommendation_notes}</p></div>}
              {histDetail.photos && histDetail.photos.length > 0 && (
                <div style={{ marginTop: 16 }}>
                  <div style={{ fontSize: 11, color: "#94a3b8", textTransform: "uppercase", marginBottom: 10 }}>Fotos</div>
